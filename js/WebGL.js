export default class WebGL {
   ...
   * @returns
   */
  IndexBufferData (type, data, usage) {
    if (!this.gl) {
      console.error('gl上下文为空');
      return;
    }
    ...
    this.gl.bufferData(type, data, usage); // 设置缓冲区数据
    return BufferId;
  }
   ...
   * @param {*} usage
   * @param {*} options 
   * options = [{ location: 0, size: 3 }]
   * location: 顶点属性位置
   * size: 顶点属性大小
   * type: 顶点属性类型
   * normalized: 顶点属性是否归一化
   * stride: 顶点属性步长
   * offset: 顶点属性偏移
   * @returns
   */
  VertexBufferData (type, data, usage, options = []) {
    if (!this.gl) {
      console.error('gl上下文为空');
      return;
    }
    ...
    // Handle default case or number for single attribute size
    if (typeof options === 'number' || options.length === 0) {
      const defaultSize = typeof options === 'number' ? options : 3;
    ...
    // Existing logic for array of options (multi-attributes)
    const typeSizes = {
      [this.gl.BYTE]: 1,
      [this.gl.UNSIGNED_BYTE]: 1,
      [this.gl.SHORT]: 2,
      [this.gl.UNSIGNED_SHORT]: 2,
      [this.gl.INT]: 4,
      [this.gl.UNSIGNED_INT]: 4,
      [this.gl.FLOAT]: 4,
      [this.gl.HALF_FLOAT]: 2, // Added for half-precision float
    };

    // 计算默认的总stride（每个顶点的总字节）
    const totalBytesPerVertex = options.reduce((acc, obj) => {
      return acc + obj.size * typeSizes[obj.type || this.gl.FLOAT];
    }, 0);
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      // 更新下一个默认offset
      if (i) {
        const attrType = options[i - 1].type || this.gl.FLOAT;
        accOffset += options[i - 1].size * typeSizes[attrType];
      }

      this.gl.vertexAttribPointer(
        option.location,
        option.size,
        option.type || this.gl.FLOAT,
        option.normalized || false,
        totalBytesPerVertex,
        accOffset
      );
      this.gl.enableVertexAttribArray(option.location);
    }
    return BufferId;
  }
